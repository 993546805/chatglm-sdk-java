...
public class ChatCompletionRequest {
    /**
     * 是否对返回结果数据做兼容,24 年 1 月发布的 GLM_3_5_TURBO  GLM_4模型,与之前的模型在返回结果上的差异.开启true可以兼容
     */
    ...
    private String requestId = String.format("thb-%d", System.currentTimeMillis());

    /**
     * do_sample 为 true 启用采样策略,do_sample 为 false 时采用策略 temperature top_p 将不生效
     * 24年 1 月发布的 GLM_3_5_TURBO GLM_4 模型时增加
     */
    ...
    public String toString() {
        try {
            // 24年1月发布新模型后调整
            if (Model.GLM_3_5_TURBO.equals(this.model) || Model.GLM_4.equals(this.model) || Model.GLM_4V.equals(this.model) || Model.GLM_4_AIR.equals(this.model) || Model.GLM_4_FLASH.equals(this.model)) {
